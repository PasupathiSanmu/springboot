...
public class EmployeeController {
	...
	public ResponseEntity<EmployeeModel> getEmployee(@PathVariable(value = "id") Long employeeId) {
		Optional<EmployeeModel> employee = service.getEmployeeById(employeeId);
 
		return new ResponseEntity<EmployeeModel>(employee.get(), HttpStatus.OK);
	...
	public ResponseEntity<EmployeeModel> updateEmployee(@PathVariable(value = "id") Long employeeId,
		...
		Optional<EmployeeModel> employee = service.getEmployeeById(employeeId);
		employee.get().setEmployeeId(employees.getEmployeeId());
		employee.get().setEmployeeName(employees.getEmployeeName());
		employee.get().setDepartmentname(employees.getDepartmentname());
		employee.get().setSalary(employees.getSalary());
		// use obj.get() syntax to pick the value of Optional type
		EmployeeModel updateEmployee = repo.save(employee.get());
